...
vcf1 <- fread("snps/23-3-15_vcf_snps_set180_encoded1.vcf.gz", header=F, sep="\t")
vcf2 <- fread("snps/23-3-15_vcf_snps_set180_encoded2.vcf.gz", header=F, sep="\t")
vcf3 <- fread("snps/23-3-15_vcf_snps_set180_encoded3.vcf.gz", header=F, sep="\t")
vcf4 <- fread("snps/23-3-15_vcf_snps_set180_encoded4.vcf.gz", header=F, sep="\t")
vcf5 <- fread("snps/23-3-15_vcf_snps_set180_encoded5.vcf.gz", header=F, sep="\t")
...
write.table(vcf_snps, "23-2-1_MEGANE_all_cichlids/23-2-16_subset/23-3-16_snps_subset_for_comparison_v3.txt",
...
up_plot_snp <- ggplot(cbind(basic_plot_snp$data,vcf_trans_full[,c("clade","pop_id")]),
    ...
    labs(title="PCA - polymorphic SNPs",x = "PC1 (2.8%)", y = "PC2 (2.3%)")+
    guides(shape = guide_legend(title = "clade"), col = guide_legend(title = "population"))+
    scale_shape_manual(values=shapes)+
    scale_color_manual(values=popPalette)
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-16_up_plot_snps_v3.pdf", width=16, height=8)
up_plot_snp
dev.off()
...
up_plot_snp_34 <- ggplot(cbind(basic_plot_snp_34$data,vcf_trans_full[,c("clade","pop_id")]),
    ...
    scale_shape_manual(values=shapes)+
    scale_color_manual(values=popPalette) 
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-16_up_plot_snp_34_v3.pdf", width=16, height=8)
up_plot_snp_34
dev.off()
