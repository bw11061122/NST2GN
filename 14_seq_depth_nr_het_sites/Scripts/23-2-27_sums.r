...
kruskal.test(sum_pol_per_sample ~ pop_id, data=meta)
sum_ins_per_sample <- colSums(vcf[type=="MEI",..meta_ids]) ### pol - polymorphisms 
sum_del_per_sample <- colSums(vcf[type=="MEA",..meta_ids]) ### pol - polymorphisms 
meta <- cbind(meta, sum_ins_per_sample, sum_del_per_sample)
kruskal.test(sum_ins_per_sample ~ pop_id, data=meta)
kruskal.test(sum_del_per_sample ~ pop_id, data=meta)
...
sum_ins_per_sample2 <- colSums(vcf_ins[,..meta_ids])
meta <- cbind(meta, sum_ins_per_sample2)
box_TEs_per_sample_ins <- ggplot(data = meta, aes(x = pop_id, y = sum_ins_per_sample2, color=clade))+
  geom_boxplot()+
  geom_jitter()+
  theme_bw()+
  scale_color_manual(values=cladePalette)+
  ylim(c(0,60000))+
  labs(title="Total number of polymorphic insertions (absent from reference)",x = "Population", y = "Sum of insertions per sample")+
  theme_bw(base_size = 26)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = margin(10, 10, 10, 100))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_MEI_sum_ins_per_sample.pdf", width=18, height=14)
...
box_TEs_per_sample_del <- ggplot(data = meta, aes(x = pop_id, y = sum_del_per_sample, color=clade))+
  ...
  ylim(c(0,60000))+
  scale_color_manual(values=cladePalette)+
  labs(title="Sum of polymorphic insertions (shared with reference)",x = "Population", y = "Sum of deletions per sample")+
  theme_bw(base_size = 26)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = margin(10, 10, 10, 100))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_MEI_sum_del_per_sample.pdf", width=18, height=14)
...
vcf_num_ins <- vcf[type=="MEI",meta_ids, with=FALSE]
...
box_TEs_per_sample_het_ins <- ggplot(data = meta, aes(x = pop_id, y = sum_het_sites_ins, color=clade))+
  ...
  scale_color_manual(values=cladePalette)+
  theme_bw()+
  labs(title="Sum of heterozygous insertions (absent from the reference)",x = "Population", y = "Heterozygous insertions per sample")+
  theme_bw(base_size = 26)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = margin(10, 10, 10, 100))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_MEI_sum_het_sites_per_sample_ins.pdf", width=18, height=12)
box_TEs_per_sample_het_ins
dev.off()

vcf_num_del <- vcf[type=="MEA",meta_ids, with=FALSE]
...
box_TEs_per_sample_het_del <- ggplot(data = meta, aes(x = pop_id, y = sum_het_sites_del, color=clade))+
  ...
  ylim(c(0,20000))+
  scale_color_manual(values=cladePalette)+
  labs(title="Sum of heterozygous insertions (shared with reference)",x = "Population", y = "Heterzoygous deletions per sample")+
  theme_bw(base_size = 26)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = margin(10, 10, 10, 100))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_MEI_sum_het_sites_per_sample_del.pdf", width=18, height=12)
...
seq_depth_by_id <- ggplot(data = meta, aes(x = seq_depth, y = sum_pol_per_sample, color=clade))+
  geom_point(size=5)+
  theme_bw()+
  scale_color_manual(values=cladePalette)+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_seq_depth_by_id_pol.pdf", width=18, height=18)
seq_depth_by_id
dev.off()
...
sum_ins_diff_by_id <- ggplot(data = meta, aes(x = seq_depth, y = sum_ins_diff, color=clade))+
  geom_point(size=5)+
  theme_bw()+
...
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_sum_ins_diff_by_id.pdf", width=18, height=16)
sum_ins_diff_by_id
dev.off()
...
sum_all_diff_by_id <- ggplot(data = meta, aes(x = seq_depth, y = sum_all_diff, color=clade))+
  geom_point(size=5)+
  theme_bw()+
  labs(title="Influence of coverage on TE identification",
  x = "Sequencing depth", y = "Difference between # heterozygous sites identified in the population and mean across populations")+
  theme_bw(base_size = 26)+
  scale_color_manual(values=cladePalette)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  geom_smooth(method='lm', formula= y~x, se=FALSE, color="black")+
  geom_smooth(data=subset(meta, seq_depth < 25), method='lm',formula=y~x,se=F, color="blue")
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_sum_all_diff_by_id.pdf", width=26, height=18)
sum_all_diff_by_id
dev.off()
...
sum_all_diff_by_id <- ggplot(data = meta, aes(x = seq_depth, y = sum_all_diff, color=pop_id))+
  ...
  geom_smooth(method='lm', formula= y~x, se=FALSE, color="black")+
  geom_smooth(data=subset(meta, seq_depth < 25), method='lm',formula=y~x,se=F, color="blue")
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-15_sum_all_diff_by_id2.pdf", width=26, height=18)
sum_all_diff_by_id
dev.off()
...
sum_het_diff_by_id_reg_ins <- ggplot(data = meta, aes(x = seq_depth, y = sum_het_diff_ins, color=pop_id))+
  ...
  geom_smooth(data=subset(meta, seq_depth < 25), method='lm',formula=y~x,se=F, color="blue")+
  #stat_cor(aes(label = after_stat(rr.label)), color = "black", geom = "label")
pdf("23-2-1_MEGANE_all_cichlids/23-2-16_subset/results_PCA/23-3-2_sum_het_diff_by_id_ins.pdf", width=26, height=18)
